  ...
  return (
    ...
    >
      ...
      <div className="mx-auto grid w-full max-w-[1200px] grid-cols-1 gap-10 lg:grid-cols-[minmax(0,550px)_minmax(0,1fr)] lg:items-stretch lg:gap-x-12 lg:gap-y-0 xl:gap-x-14">
        <AnimateOnView className="flex min-h-[280px] w-full min-w-0 flex-col lg:min-h-0 lg:h-full">
        ...
        <div className="flex min-h-0 min-w-0 flex-col gap-8 lg:h-full lg:justify-start">
          <div className="flex w-full max-w-[640px] flex-col gap-8 xl:max-w-2xl">
            ...
            <AnimateOnView delayMs={240} className="flex w-full max-w-full flex-col gap-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1">
                  <span className="font-poppins text-3xl font-bold text-white xl:text-5xl">
                    {bookProgress ? (
                      <CountUpText value={collected} durationMs={1600} />
                    ) : (
                      '--'
                    )}
                  </span>
                ...
                <div className="flex items-center gap-2 text-white">
                  ...
                  <span className="font-poppins text-base font-medium xl:text-[28px]">
                    {bookProgress && bookProgress.pricePerBook > 0 ? (
                      <>
                        N
                        <CountUpText
                          value={bookProgress.pricePerBook}
                          durationMs={1600}
                        />
                        /BOOK
                      </>
            ...
            <AnimateOnView
              delayMs={320}
              className="flex w-full max-w-full flex-col gap-4 sm:flex-row sm:items-center"
            >
              <Link
                href="/donate"
                className={`flex h-10 w-full items-center justify-center rounded-xl bg-[#E63715] font-medium text-white transition-colors hover:bg-[#cf3212] xl:h-16 xl:text-2xl ${hoverLiftClass}`}
              >
                Donate Now
              </Link>
              <Link
                href="/build-a-reader"
                className={`flex h-10 w-full items-center justify-center rounded-xl border border-white bg-transparent font-medium text-white transition-colors hover:border-white/90 hover:bg-white/10 xl:h-16 xl:text-2xl ${hoverLiftClass}`}
              >
                Learn More
              </Link>
            </AnimateOnView>
